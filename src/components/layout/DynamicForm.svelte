<script>
    ...
    [
        ...
        [{prop: "color", label: "Color of the thing", type: "color"}
    ]
    */
    export let form = [];
</script>
        ...
        {#each row as col}
            <div class="field column mb-0 {col.class || ""}">
                <div class="control">
                    <label class="label"
                        >
                        <span class="field-name">{col.label || col.prop}</span>
                        {#if typeof value[col.prop] == "object" && value[col.prop] instanceof Date}
                            <input
                                class="input"
                                type="date"
                                value={value[col.prop]?.toISOString().split("T")[0]}
                                on:change={(e) =>
                                    (value[col.prop] = parseDate(e.target.value))}
                            />
                        ...
                        {:else if typeof value[col.prop] === "string"}
                            {#if col.type == "color"}
                                <ColorInput bind:color={value[col.prop]} availableColors={colors}/>
                            {:else}
                                <input
                                    class="input"
                                    type="text"
